...
- [ ] **T-49**: Verify Dell network config
  ...
  - **Diagnosis (Feb 23)**: NIC hardware confirmed working — gigabit link negotiated via direct MBP-to-Dell Ethernet. Ping sweep from SHANNON and MBP: Dell invisible on all IPs (.1-.254). ARP table empty. Conclusion: kernel loaded NIC driver (link negotiation) but userspace network stack not running (NetworkManager/systemd-networkd not configuring interface).
  - **Laundry room power outage** — circuit breaker tripped (3 fridges overloading circuit). Chest freezer unplugged to reduce load.
  - **Diagnosis (Feb 24)**: (1) GPU HDMI confirmed dead — two cables tried. (2) CMOS reset — no change. (3) GPU DP works — GPU alive, only HDMI fried. (4) BIOS: "No Boot Device Found." (5) SSD pulled and tested externally — initially all tests failed, but discovered Mac Mini back USB port is flaky (no device enumerates there). Re-tested via Acer monitor USB passthrough: D-Lock adapter works (160GB HDD detected), but Dell SSD not detected on either working adapter (D-Lock or CURIE enclosure). **SSD confirmed dead.** LED flashes rapidly = SATA handshake retry loop. (6) Borrowed DP cable returned. Full report: `~/dotfiles/docs/INCIDENT_REPORT_DELL_POWER_SURGE_2026_02_23.md`.
  - **Next steps (revised Feb 25 evening)**: ~~860 EVO TVS check~~ RULED OUT — multimeter shows no shorted protection components, controller damage confirmed. Ubuntu Server 24.04.4 LTS confirmed fully compatible (all Dell 7010 hardware works out of box). ISO downloaded. (1) **PSU multimeter check** — GPU out, motherboard DP for visual BIOS access, probe SATA power voltages with no drives connected. (2) Flash "Jocke" (160GB HDD) with Ubuntu via Mac `dd` (no USB stick needed — hybrid ISO). (3) Boot Dell from Jocke (PSU canary + temp OS + smartctl). (4) SMART-check FERMI from Dell Linux. (5) FERMI as permanent Dell drive. (6) `kamal deploy` restores brf-auto from git (~30 min). (7) Get surge protector/UPS. See `~/dotfiles/docs/INCIDENT_REPORT_DELL_POWER_SURGE_2026_02_23.md` for full recovery plan, Ubuntu compatibility matrix, and data loss assessment.
