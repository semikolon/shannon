...
- [ ] **T-49**: Verify Dell network config
  ...
  - **Next steps (revised Feb 25)**: ~~860 EVO TVS check~~ RULED OUT — multimeter shows no shorted protection components, controller damage confirmed. (1) **PSU safety check** — 160GB HDD as sacrificial canary in Dell SATA bay. (2) Install Ubuntu Server 24.04 LTS on 160GB HDD via headless autoinstall (triple duty: PSU canary + temp OS + smartctl). (3) SMART-check all SSDs from Dell Linux. (4) Permanent drive: new SSD (~300-500 SEK) or FERMI based on SMART. (5) `kamal deploy` restores brf-auto from git (~30 min). (6) Get surge protector/UPS. (7) Ask Gustav about Synology NAS for temp storage. See `~/dotfiles/docs/INCIDENT_REPORT_DELL_POWER_SURGE_2026_02_23.md` for full recovery plan and data loss assessment.
